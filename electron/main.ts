...
function createWindow() {
  ...
  const devServerURL = VITE_DEV_SERVER_URL || (isDev ? 'http://localhost:5173' : undefined)
  ...
  if (devServerURL) {
    ...
    const tryLoad = () => {
      win?.loadURL(devServerURL).catch(err => {
        console.error('[Main] Failed to load dev server URL, retrying...', err)
        // 如果加载失败，等待 1 秒后重试
    ...
    win.loadFile(indexPath).catch(err => {
      console.error('[Main] Failed to load local file:', err)
    })
  }
