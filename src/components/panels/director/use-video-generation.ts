...
export async function callVideoGenerationApi(
  ...
  console.log('[VideoGen] Detected API format:', { model, format, platform: resolvedPlatform, baseUrl: videoBaseUrl });

  // 云雾API特殊处理：检查是否需要不同的Authorization格式
  const isYunwu = resolvedPlatform === 'yunwu' || videoBaseUrl.includes('yunwu');
  if (isYunwu) {
    console.log('[VideoGen] Detected Yunwu API, using special handling');
  }
...
async function callUnifiedVideoApi(
  ...
  const submitUrls = [
    `${cleanBaseUrl}/v1/video/generations`,
    `${cleanBaseUrl}/v1/video/create`,
  ];
  ...
  for (const submitUrl of submitUrls) {
    console.log('[VideoGen] Attempting video generation at:', submitUrl);
    console.log('[VideoGen] Request body:', JSON.stringify(body, null, 2));
    console.log('[VideoGen] Is Yunwu API:', isYunwu);
    
    // 云雾API可能需要不同的Authorization格式，先尝试标准Bearer格式
    const headers: Record<string, string> = {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    };
    
    // 云雾API可能使用不同的Authorization格式
    if (isYunwu) {
      // 尝试多种格式：Bearer、直接key、或X-API-Key
      headers['Authorization'] = `Bearer ${apiKey}`;
      // 如果Bearer格式失败，会在错误处理中尝试其他格式
    } else {
      headers['Authorization'] = `Bearer ${apiKey}`;
    }
    ...
    console.error('[VideoGen] Video API error:', {
      status: resp.status,
    ...
    // 云雾API特殊处理：如果是401/403且错误信息包含"Invalid API"，尝试其他Authorization格式
    if (isYunwu && (resp.status === 401 || resp.status === 403) && errorText.includes('Invalid API')) {
      console.log('[VideoGen] Yunwu API: Trying alternative Authorization format');
      // 尝试直接使用API key（不带Bearer）
      const altHeaders: Record<string, string> = {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'Authorization': apiKey, // 不带Bearer前缀
      };
      
      // 使用cleanBaseUrl构建备用URL
      const altSubmitUrl = submitUrl.replace(baseUrl, cleanBaseUrl);
      const altResp = await fetch(altSubmitUrl, {
        method: 'POST',
        headers: altHeaders,
        body: JSON.stringify(body),
      });
      ...
      if (altResp.ok) {
        submitData = await altResp.json();
        submitError = null;
        console.log('[VideoGen] Yunwu API: Alternative format succeeded');
        break;
      }
    ...
    if (resp.status === 404 || resp.status === 405) {
      submitError = new Error(`视频 API 错误: ${resp.status} - ${errorText.substring(0, 200)}`);
...
async function callVolcVideoApi(
  ...
  if (!submitResponse.ok) {
    ...
    console.error('[VideoGen] Volc video submit error:', {
    ...
    // 云雾API特殊处理：如果是401/403且错误信息包含"Invalid API"，尝试其他Authorization格式
    if (isYunwu && (submitResponse.status === 401 || submitResponse.status === 403) && errorText.includes('Invalid API')) {
      console.log('[VideoGen] Yunwu API: Trying alternative Authorization format for Volc endpoint');
      ...
      if (altResp.ok) {
        ...
        console.log('[VideoGen] Yunwu API: Alternative format succeeded for Volc');
        const taskId = altData.id?.toString();
        if (!taskId) throw new Error('返回空的任务 ID');
    ...
    body: JSON.stringify(requestBody),
      ...
      if (keyHeaderResp.ok) {
        ...
        const taskId = keyData.id?.toString();
        if (!taskId) throw new Error('返回空的任务 ID');
...
async function pollVolcTaskWithAuth(
  apiKey: string,
  baseUrl: string,
  taskId: string,
  onProgress?: (progress: number) => void,
  ...
  const getHeaders = () => {
    ...
    if (authFormat === 'Bearer') {
      headers['Authorization'] = `Bearer ${apiKey}`;
    } else if (authFormat === 'direct') {
      headers['Authorization'] = apiKey;
    } else if (authFormat === 'X-API-Key') {
