...
async function uploadWithProvider(
  ...
  try {
    ...
    if (!response.ok) {
      let message = data?.error?.message || data?.message || text || `上传失败: ${response.status}`;
      
      // 特殊处理：如果错误信息包含"Invalid API"或"Invalid key"，提供更友好的提示
      if (message.includes('Invalid API') || message.includes('Invalid key') || message.includes('Invalid API key')) {
        // 检查API key格式是否正确（imgbb的key应该是32位十六进制）
        const isImgbbKey = provider.id === 'imgbb';
